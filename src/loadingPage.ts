...
export class loadingPage extends PIXI.Container {
  ...
  async PreloadAssets() {
    ...
    const preloader_bg = new PIXI.Sprite(
      Assets.get("Loading_Screen_Background"),
    );
    ...
    Game_Logo.position.set(
      AppDimension.width / 2,
      AppDimension.height / 2 - 85,
    );
    this.addChild(Game_Logo);

    const LoadingBarContainer = new PIXI.Container();
    LoadingBarContainer.position.set(
      AppDimension.width / 2,
      AppDimension.height / 2 + 150,
    );
    this.addChild(LoadingBarContainer);

    const loadingBar_empty = await new PIXI.Sprite(
      Assets.get("Loading_bar_empty_1"),
    );
    loadingBar_empty.anchor.set(0.5);
    LoadingBarContainer.addChild(loadingBar_empty);

    const loadingbar_fill = await new PIXI.Sprite(
      Assets.get("Loading_bar_fill_2"),
    );
    ...
    LoadingMask.fill(0x000000)
      .rect(58, -25, loadingbar_fill.width - 140, 50)
      .endFill();
    ...
    const loadingBar_design = await new PIXI.Sprite(
      Assets.get("Loading_bar_design_3"),
    );
