...
export class ReelsGrid extends PIXI.Container {
    ...
    createMainReels() {
        ...
        for (let col = 0; col < this.cols; col++) {
            ...
            for (let row = 0; row < this.rows+ReelProperties.dummyRows; row++) {
                ...
                symbolContainer.x = col * this.reelWidth - ((this.cols - 1) * this.reelWidth) / 2;
                symbolContainer.y = row * this.symbolHeight - (((this.rows+ReelProperties.dummyRows) - 1) * this.symbolHeight) / 2;
        ...
        this.reelendY = this.reels[0][this.rows+ReelProperties.dummyRows-1].y+this.symbolHeight;
    ...
    initiate_reel_rotation() {  
        for (let col = 0; col < this.cols; col++) {
            ...
            const delay = 0//(col+row) * 0.2; // stagger reel start
                gsap.to(this.reels[col][row],{duration:1,delay:1/(col+row),y:this.reelendY,onComplete:(row,col)=>{
                    if(row>=this.rows){
    ...
    rotateDummyreels(row:number,col:number){
        ...
        gsap.fromTo(this.reels[col][row], {y:this.reelstartY}, {delay:delay,y:this.reelendY, duration: 1, ease: "none", onComplete: (row,col) => {
            // if(row==this.rows){
            // setTimeout(() => {
                this.rotateDummyreels(row, col)                
